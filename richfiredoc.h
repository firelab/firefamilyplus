...
class CRichFireDoc : public CRichEditDoc
{
protected: // create from serialization only
	CRichFireDoc();
	~CRichFireDoc();
	DECLARE_DYNCREATE(CRichFireDoc)

// Attributes
public:
	//bool conditional;
	void SetFires(CFireStats *_fires);
	void SetLargeFiresPerDay(int _nEntries, int *_vals);
	bool isTemp;
	CString tName;
	//int m_nNewDocType;
...
// Implementation
public:
	CString discardFileName;
	bool hasDiscards;
	bool killStats;
	CFireStats * fires;
	CFireplusSet *fpSet;
	int maxNumLargeFiresPerDay;
	int *numLargeFiresPerDay;
...
protected:
	...
	//}}AFX_MSG
	afx_msg void OnDecisionPoint();
	DECLARE_MESSAGE_MAP()
public:
	afx_msg void OnDataAddtocandidateslist();
	afx_msg void OnViewFsimlargefiredistfile();
};
