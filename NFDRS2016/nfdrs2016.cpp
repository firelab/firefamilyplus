...
void NFDR2016Calc::Init(double inLat, char iFuelModel, int inSlopeClass, double inAvgAnnPrecip, bool LT, bool Cure, bool isAnnual, int kbdiThreshold)//, double fMaxGSI , double fGSIGreenupThreshold )
{
	CTA = 0.0459137;
	NFDRSVersion = 16;                                          // NFDRS Model Version
	Lat = inLat;                                                // Latitude (degrees)
   ...
   // Update live fuel moisture once per day
    if (Hour == RegObsHr)// || num_updates==0)
    {
       ...
       // WoodyFM.Update(RH, Temp, MinTemp, Julian);
		...
		GsiFM.Update(Temp, MaxTemp, MinTemp, RH, MinRH, Julian, GetXDaysPrecipitation(GsiFM.GetNumPrecipDays()), thisTime);
		HerbFM.Update(Temp, MaxTemp, MinTemp, RH, MinRH, Julian, GetXDaysPrecipitation(HerbFM.GetNumPrecipDays()), thisTime);
		WoodyFM.Update(Temp, MaxTemp, MinTemp, RH, MinRH, Julian, GetXDaysPrecipitation(WoodyFM.GetNumPrecipDays()), thisTime);
