// FireSet.h : Implementation of the CFireSet class

 

// CFireSet implementation

// code generated on Wednesday, September 28, 2005, 2:01 PM

#include "stdafx.h"
#include "FireSet.h"
IMPLEMENT_DYNAMIC(CFireSet, CRecordset)

CFireSet::CFireSet(CDatabase* pdb)
	: CRecordset(pdb)
{
	m_FireID = 0;
	m_Year = L"";
...
CString CFireSet::GetDefaultConnect()
{
...
CString CFireSet::GetDefaultSQL()
{
	return _T("[Fire]");
}

void CFireSet::DoFieldExchange(CFieldExchange* pFX)
{
...
// ODBC will try to automatically convert the column value to the requested type
	...
	RFX_Text(pFX, _T("[Slope]"), m_Slope);
	RFX_Text(pFX, _T("[Elevation]"), m_Elevation);
	RFX_Text(pFX, _T("[Aspect]"), m_Aspect);
	RFX_Text(pFX, _T("[FuelModel]"), m_FuelModel);
	RFX_Text(pFX, _T("[FireType]"), m_FireType);

}
/////////////////////////////////////////////////////////////////////////////
// CFireSet diagnostics

#ifdef _DEBUG
void CFireSet::AssertValid() const
{
	CRecordset::AssertValid();
}

void CFireSet::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
