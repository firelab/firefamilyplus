...
public:
	...
	inline void SetFrontmost() const
	{
		if (::IsWindow(this->hThreadWnd) && this->bTerminate == false)
			BringWindowToTop(this->hThreadWnd);
			//::SendMessage(this->hThreadWnd, WM_SHOWWINDOW, 0, (LPARAM)1);
	}

	inline void ShowProgressBar(bool bShowProgreesBar) const
	{
		if (::IsWindow(this->hThreadWnd))
			::SendMessage(this->hThreadWnd, bShowProgreesBar ? _ProgressThreadData::WM_SHOWPROGRESSBAR : _ProgressThreadData::WM_HIDEPROGRESSBAR, 0, 0);
	}

	inline HWND GetHWND() const
	{
		return this->hThreadWnd;
	}
};
