...
IMPLEMENT_DYNAMIC(CWxStationSet, CRecordset)

CWxStationSet::CWxStationSet(CDatabase* pdb)
	: CRecordset(pdb)
{
	m_StationID = L"";
	m_nFields = 1;
...
CString CWxStationSet::GetDefaultConnect()
{
...
CString CWxStationSet::GetDefaultSQL()
{
	return _T("[WXObs]");
}

void CWxStationSet::DoFieldExchange(CFieldExchange* pFX)
{
...
#ifdef _DEBUG
void CWxStationSet::AssertValid() const
{
	CRecordset::AssertValid();
}

void CWxStationSet::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
