...
IMPLEMENT_DYNCREATE(CFireStatFrame, CMDIChildWnd)

CFireStatFrame::CFireStatFrame()
{
}
...
BEGIN_MESSAGE_MAP(CFireStatFrame, CMDIChildWnd)
	//{{AFX_MSG_MAP(CFireStatFrame)
	ON_COMMAND(ID_FILE_PRINT_PRINTGRAPH, OnFilePrintPrintgraph)
	ON_COMMAND(ID_FILE_SAVE_GRAPH, OnFileSaveGraph)
	ON_COMMAND(ID_VIEW_DISCARDEDFIRES, OnViewDiscardedfires)
...
/////////////////////////////////////////////////////////////////////////////
// CFireStatFrame message handlers

BOOL CFireStatFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
	if (!m_wndStatusBar.Create(this) ||
		!m_wndStatusBar.SetIndicators(cfsindicators,
		  sizeof(cfsindicators)/sizeof(UINT)))
	{
...
/*	if (!m_wndSplitter2.CreateStatic(
	...
	//CFireStatGraph *tmpView = (CFireStatGraph *)m_wndSplitter.GetPane(0,1);
	//tmpView->fires = ((CRichFireDoc*)pContext->m_pCurrentDoc)->fires;
	return TRUE;//CMDIChildWnd::OnCreateClient(lpcs, pContext);
}

void CFireStatFrame::MaximizePane(CView * view)
{
...
void CFireStatFrame::OnFilePrintPrintgraph() 
{
	// TODO: Add your command handler code here
	((CFireStatGraph *)m_wndSplitter.GetPane(0, 1))->PrintGraph();
	
}

void CFireStatFrame::OnFileSaveGraph() 
{
	...
	if(fd.DoModal() == IDOK)
	{
		...
		((CFireStatGraph *)m_wndSplitter.GetPane(0, 1))->DrawToDCRegion(&memhdc, rect);
		char fName[256];
...
void CFireStatFrame::OnViewDiscardedfires() 
{
	CRichFireDoc* pDoc = (CRichFireDoc*)GetActiveDocument();
	if(pDoc && pDoc->hasDiscards && pDoc->discardFileName.GetLength() > 0)
	{
		...
		while(curTemplatePos != NULL)
		{
			...
			if(str == _T("TextFile"))
			{
				CRichDoc *nDoc = (CRichDoc *)	curTemplate->OpenDocumentFile(pDoc->discardFileName);
				nDoc->isTemp = false;
				nDoc->SetTitle("Discarded Fires");
				return;
			}
		}
	}
	else
		AfxMessageBox("No discarded fires file available");
}

void CFireStatFrame::OnFileSaveAs() 
{
	((CRichView *)m_wndSplitter.GetPane(0, 0))->SaveTheDamnThing();
}

void CFireStatFrame::OnFilePrintReport() 
{
	((CRichView *)m_wndSplitter.GetPane(0, 0))->PrintTheDamnThing();
}
// added for batch2
void	CFireStatFrame::OnViewHeader()
{
	CRichFireDoc* pDoc = (CRichFireDoc*)GetActiveDocument();
	fpSet = pDoc->fpSet;
