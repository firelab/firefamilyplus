...
short CNFDRS::iCalcIndexes(short iWS, short iSlopeCls, double * fROS, double * fSC, double * fERC, double * fFL, double * fFIL, double * fBI)
{
	if (m_nfdrs)
	{
		int iFIL, iBI, iSC;
		int ret = m_nfdrs->iCalcIndexes(iWS, iSlopeCls, fROS, &iSC, fERC, fFL, &iFIL, &iBI);
		*fSC = iSC;
		*fBI = iBI;
		*fFIL = iFIL;
		return ret;
...
short CNFDRS::iCalcIC(short iTemp, short iSOW, double fMC1, double fROS, double * fIC)
{
	if (m_nfdrs)
	{
		int iIC;
		int ret = m_nfdrs->iCalcIC(iTemp, iSOW, fMC1, fROS, &iIC);
		*fIC = iIC;
		return ret;
	}
	return -1;
...
short CNFDRS::iCalcMoistWetFlagSnowFlag(short iInit, short iTemp, short iRH, short iMaxTemp, short iMaxRH, 
...
{
	if(m_nfdrs)
		return m_nfdrs->iCalcMoistWetFlagSnowFlag(iInit, iTemp, iRH, iMaxTemp, iMaxRH, 
								iMinTemp, iMinRH, iPrecipDur, fPrecipAmt, iSOW, f10stick, 
								iJulian, iYear, iDeclareGreenUp, iDeclareFreeze, iGreenHerb, 
								iGreenWoody, iSeason, iWetFlag, iSnowFlag, f1, f10, f100, 
								f1000, fHerb, fWood, iStage, fGren, 
								fX1000, iRainEvent, iKBDI);
	return -1;
}

short CNFDRS::iCalcMoistWetFlagSnowFlag2(short iInit, short iTemp, short iRH, short iMaxTemp, short iMaxRH, 
								short iMinTemp, short iMinRH, short iPrecipDur, double fPrecipAmt, short iSOW, double f10stick, 
								short iJulian, short iYear, short iDeclareGreenUp, short iDeclareFreeze, short iGreenHerb, 
								short iGreenWoody, short iSeason, int iWetFlag, int iSnowFlag, double * f1, double * f10, int * iStage, double * fGren, 
								double * fX1000, int * iRainEvent)
{
	if(m_nfdrs)
		return m_nfdrs->iCalcMoistWetFlagSnowFlag2(iInit, iTemp, iRH, iMaxTemp, iMaxRH, 
								iMinTemp, iMinRH, iPrecipDur, fPrecipAmt, iSOW, f10stick, 
								iJulian, iYear, iDeclareGreenUp, iDeclareFreeze, iGreenHerb, 
								iGreenWoody, iSeason, iWetFlag, iSnowFlag, f1, f10,   iStage, fGren, 
								fX1000, iRainEvent);
	return -1;
}
...
short CNFDRS::iCalcFFWI (short iTemp, short iRH, short iWindSpeed)
{
	if(m_nfdrs)
		return m_nfdrs->iCalcFFWI(iTemp, iRH, iWindSpeed);
	return -1;
}
